var appName = window.location.pathname.split('/').length > 3 ? window.location.pathname.split('/')[1] : '';
...
$(document).ready(function () {
    var $window = $(window).on('resize', function () {
        ...
        var $footer = $('#footer');
        var $content = $('#divSVG');
        var $userName = $('#divUsername');
        var height = $(this).height() - ($header.height() + $footer.height() + $userName.height() + 70);
    ...
    //$.ajax({
    //    type: "POST",
    //    url: window.location.origin + "/" + appName + "/WebMethods/CommonWebMethods.asmx/CheckAdminAccess",
    //    contentType: "application/json; charset=utf-8",
    //    dataType: "json",
    //    success: function (result) {
    //        if (result.d == 'Admin') {
    //            $('#liSwitchToAdmin').css('display', 'block');
    //        }
    //        else {
    //            $('#liSwitchToAdmin').css('display', 'none');
    //        }
    //    },
    ...
    //Displaying Support Details Popup
    ...
    //$('#dtMtbDate').data('DateTimePicker').minDate(new Date());
    ...
    $(".aDataEvolution").click(function (e) {
...
//For Menu Item Navigation
    var viewMenuDetails = function (clickedOption) {
        var navigateTo = '';
        switch (clickedOption.toLowerCase().replace(/\s/g, '')) {
            case "listebs":
                navigateTo = "/Pages/EBSListing";
                break;
            case "dataqualityindex":
                navigateTo = "/Pages/DataQualityStatistics";
                break;
            case "switchtoadmin" :
        ...
        window.location.href = window.location.origin + "/" +appName +navigateTo;
        };
...
var handleSidebarMinify = function () {
    $('[data-click=sidebar-minify]').click(function (e) {
        ...
        if ($(targetContainer).hasClass(sidebarClass)) {
            $(targetContainer).removeClass(sidebarClass);
            $('#lblModuleHeader').css('padding-left', '280px');
            ...
            $(targetContainer).addClass(sidebarClass);
            $('#lblModuleHeader').css('padding-left', '80px');
            if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
                if ($(targetContainer).hasClass('page-sidebar-fixed')) {
                    $('#sidebar [data-scrollbar="true"]').slimScroll({ destroy: true });
                    $('#sidebar [data-scrollbar="true"]').removeAttr('style');
...
/* For removing background for EBS listing and Data Quality Pages
------------------------------------------------ */
window.onload = function () {
    if (window.location.pathname.indexOf('EBSListing') > 0) {
        $('body').removeClass('adminBody');
    }
    if (window.location.pathname.indexOf('DataQualityStatistics') > 0) {
        $('body').removeClass('adminBody');
    }
    else if (window.location.pathname.indexOf('EBSListing') < 0) {
